...
echo:
echo %WARN% PREREQS: Checking prerequisites.
echo %WARN% PREREQS: Inspect results and make sure that all tests are OK and no ERRORs reported before continuing.
echo %INFO%
...
echo %WARN% PREREQS - MS Build Tools

if not defined VSINSTALLDIR (
  echo %INFO% PREREQS - MS Build Tools: Activated environment not found - variable VSINSTALLDIR is not set.
  if exist "%~dp0..\MSBuildTools\VC\Auxiliary\Build\vcvarsall.bat" (
    echo %OKOK% PREREQS - MS Build Tools: Found environment in "%~dp0..\MSBuildTools".
  ) else (
    if exist "%~d0\dev\MSBuildTools\VC\Auxiliary\Build\vcvarsall.bat" (
      echo %OKOK% PREREQS - MS Build Tools: Found environment in "%~d0\dev\MSBuildTools".
    ) else (
      echo %ERROR% PREREQS - MS Build Tools: Environment not found in
      echo %ERROR% PREREQS - MS Build Tools:   "%~d0\dev\MSBuildTools"
      echo %ERROR% PREREQS - MS Build Tools:   "%~dp0..\MSBuildTools"
    )
  )
) else (
  if exist "%VSINSTALLDIR%\VC\Auxiliary\Build\vcvarsall.bat" (
    echo %OKOK% PREREQS - MS Build Tools: Found activated environment in "%VSINSTALLDIR%" is not set.
  ) else (
    echo %ERROR% PREREQS - MS Build Tools: Variable VSINSTALLDIR is set: "%VSINSTALLDIR%", but
    echo %ERROR% PREREQS - MS Build Tools: main script "%%VSINSTALLDIR%%\VC\Auxiliary\Build\vcvarsall.bat"
    echo %ERROR% PREREQS - MS Build Tools: is not found. Enviroment is corrupted.
  )
)
echo:
